import { periodicRate, interestOverPeriods, buildSchedule, type ScheduleRow } from '../mortgage/amortization';
 ...
 * amortization engine (`services/mortgage/amortization.ts`) rather than
...
export function computeInstallmentLoanSummary(input: LoanSummaryInput): LoanSummary {
  ...
  // 0% APR means zero interest BY DEFINITION — never derive it from `pmt·N −
  // principal`. That formula assumes payment×term reconciles exactly to
  // principal; a real recorded payment (rounding, a folded-in fee) rarely
  // does, and treating the mismatch as "interest" would misreport a 0% BNPL
  // plan as interest-bearing.
 ...
 *
 * `buildSchedule()` (services/mortgage/amortization.ts) works in DOLLARS —
 * its `roundCents()` helper assumes dollar-scale input, so cents must be
 * converted to dollars before calling it and back to cents on the way out.
 * Mirrors `mortgage-service.ts`'s `getSchedule()` unit-conversion pattern.
 */
