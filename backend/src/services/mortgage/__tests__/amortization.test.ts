...
describe('interestOverPeriods — forecast tail / scenario split', () => {
  const i = periodicRate(0.05, 12, 'semi_annual');
  ...
  it('splits N payments into interest vs principal exactly against balanceAfter', () => {
    ...
    expect(r.principal).toBeCloseTo(500_000 - r.endBalance, 6);
    // Interest + principal repaid = cash paid over the span.
    expect(r.interest + r.principal).toBeCloseTo(pmt * 12, 6);
    // Early in a 5% mortgage, interest dominates principal.
  ...
  it('a zero-length span is a no-op; the balance never goes negative', () => {
    ...
    // A payment that overshoots the balance clamps endBalance to 0 (no negative);
    // interest is never negative.
    const big = interestOverPeriods(1_000, i, 100_000, 1);
    expect(big.endBalance).toBe(0);
    expect(big.interest).toBeGreaterThanOrEqual(0);
  });
});
