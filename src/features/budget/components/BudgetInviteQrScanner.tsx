...
type Props = {
  ...
  onClose: () => void;
  /**
   * The raw decoded payload — a `simplebudget://lf-invite?…` link when the code
   ...
   * sentence to the person joining ("that is not a Symply Budget invite"), not
   * a null this component could do anything useful with.
   */
 ...
 * still; without the latch below, the join confirmation would be re-opened
 * (and the fields refilled) continuously and the sheet would appear to freeze.
 * Health's barcode camera debounces instead, because there the same code
 * genuinely may be scanned again after a lookup — here a scan navigates, so
 * once is the whole contract.
 *
 * The scanner is a convenience, never the only way in: every failure path here
 * — no permission, no camera, a code from some other app — ends by pointing at
 * the code and secret fields behind it, which are always available.
 */
export function BudgetInviteQrScanner({ visible, onClose, onScanned }: Props) {
  ...
  return (
    <Modal
      ...
      testID="budget-invite-scanner-modal"
    >
      <View style={styles.root} testID="budget-invite-scanner">
        {granted ? (
          <CameraView
            ...
            // QR only. Every other symbology this camera can read is something
            // we would have to reject anyway, and narrowing the set keeps a
            // barcode on a nearby cereal box from claiming the one scan.
            barcodeScannerSettings={{ barcodeTypes: ['qr'] }}
            onBarcodeScanned={handleBarcodeScanned}
            testID="budget-invite-scanner-camera"
          />
        ) : (
        ...
        <View style={[styles.topBar, { paddingTop: insets.top + Spacing.sm }]}>
          <Pressable
            ...
            testID="budget-invite-scanner-close"
          ...
          {granted ? (
            <Pressable
              ...
              testID="budget-invite-scanner-torch"
              style={styles.iconButton}
              hitSlop={12}
            >
              <Icon
        ...
        {granted ? (
          <View style={styles.guideLayer} pointerEvents="none">
            ...
            <Typography variant="caption2" color="rgba(255,255,255,0.75)" style={styles.instruction}>
              Symply Budget → Invite & Household → Invite → Generate QR code.
            </Typography>
          </View>
        ) : null}
        ...
        {mountError !== null ? (
          <View
            ...
            testID="budget-invite-scanner-error"
          >
...
function PermissionGate({
  ...
  if (permission === null) {
    return (
      <View style={styles.center} testID="budget-invite-scanner-checking">
  ...
  return (
    <View style={styles.center} testID="budget-invite-scanner-permission-denied">
      ...
      <Typography variant="footnote" color="rgba(255,255,255,0.8)" style={styles.permissionBody}>
        Symply Budget uses the camera only to read the invite code you are shown. You can close this
      ...
      <Pressable
        ...
        testID={
          permission.canAskAgain
            ? 'budget-invite-scanner-request-permission'
            : 'budget-invite-scanner-open-settings'
        }
...
export default BudgetInviteQrScanner;
