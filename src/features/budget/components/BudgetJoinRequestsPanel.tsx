...
import type { BudgetJoinRequests } from '@features/budget/local/useBudgetJoinRequests';
 ...
 *
 * Rendered ABOVE everything else on whichever screen shows it: a pending
 * request is the only thing in this flow that is WAITING on the person reading
 * it, and it used to sit inside a collapsed section they had to know to tap.
 *
 ...
 */
export function BudgetJoinRequestsPanel({ requests, sas, isApproving, approve }: BudgetJoinRequests) {
  const colors = useAppColors();
  if (!requests || requests.length === 0) return null;

  return (
    <View style={styles.stack} testID="budget-settings-join-requests-panel">
      ...
      {requests.map((request) => {
        ...
        return (
          <Card
            ...
            testID={`budget-join-request-banner-${request.inviteId}`}
          >
            <View style={styles.head}>
              ...
              <View style={styles.headText}>
                ...
                <Typography
                  variant="footnote"
                  weight="semibold"
                  testID={`budget-request-who-${request.inviteId}`}
                >
                  {who}
                </Typography>
                <Typography variant="caption2" color={colors.textSecondary}>
            ...
            {digits ? (
              <>
                ...
                <Typography
                  ...
                  weight="semibold"
                  style={styles.sas}
                  testID={`budget-request-sas-${request.inviteId}`}
                >
                ...
                <GradientButton
                  ...
                  testID="budget-settings-approve-request"
                />
              </>
            ) : (
              ...
              <Typography
                ...
                testID={`budget-request-sas-missing-${request.inviteId}`}
              >
