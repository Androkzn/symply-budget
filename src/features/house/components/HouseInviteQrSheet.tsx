...
type Props = {
  ...
  onClose: () => void;
  /**
   * The `simplehouse://lf-invite?…` link the code encodes — the SAME string the
   * share sheet sends, so the two paths cannot drift apart.
   *
 ...
 *
 * Why a QR at all, when the invite already had a link and a share sheet: the two
 * people doing this are usually in the same room. Sharing means picking a
 * messenger, finding the person, sending a link that carries the home secret
 * through a third party's servers — for a hand-off that could be one phone
 * pointed at another. The code encodes exactly the same link, so:
 *
 *  - the invitee's Symply House can scan it (Join → Scan QR code), and
 ...
 *    hands it to the app, and `captureHouseInviteLink` fills the join form.
 ...
 * exist yet.
 */
export function HouseInviteQrSheet({
  visible,
  onClose,
  link,
  shortCode,
  ...
  return (
    <BottomSheet
      ...
      showCloseButton
    >
      <View style={styles.body} testID="house-invite-qr-sheet">
        {link && !failed ? (
          <>
            {/*
            ...
            */}
            <View style={styles.plate} testID="house-invite-qr-plate">
              <QRCode
                value={link}
                size={qrSize}
                color="#000000"
        ...
        <View style={[styles.values, { borderTopColor: colors.borderColor }]}>
          <View style={styles.valueRow}>
            ...
            <Typography variant="footnote" weight="semibold" testID="house-invite-qr-code">
              {shortCode}
            </Typography>
          </View>
          <View style={styles.valueRow}>
        ...
        {boundToEmail ? (
          <Typography
            variant="caption2"
            color={colors.textSecondary}
            testID="house-invite-qr-bound-to"
          >
            Only {boundToEmail} can use this invite.
          </Typography>
        ) : null}
        {expiresIn ? (
          <Typography variant="caption2" color={colors.textTertiary} testID="house-invite-qr-expiry">
            Expires {expiresIn}.
          </Typography>
        ) : null}
      </View>
...
const styles = StyleSheet.create({
  ...
  // modules, and a code drawn flush to the edge of a coloured card is one of the
  // more common reasons a valid QR will not scan.
  plate: {
    alignSelf: 'center',
    backgroundColor: '#FFFFFF',
...
export default HouseInviteQrSheet;
