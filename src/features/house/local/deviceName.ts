 ...
 *
 * The control plane has always carried a `label` per device — House's
 * registration just sent the constant `'Primary'` and the UI never read it, so
 * Devices showed `Device 8b0240` and nobody could tell which phone was which.
 *
 * Two sources, in order: whatever the member typed here (kept locally and
 * re-sent on every registration, so a rename survives a reinstall of the peer's
 ...
 * readable.
 *
 * Its own key, not Budget's: the two apps can be installed side by side, and a
 * device renamed in one must not silently rename itself in the other.
 */

export const HOUSE_DEVICE_NAME_KEY = 'house.local.deviceName.v1';
...
export async function getDeviceNameOverride(): Promise<string | null> {
  const stored = normalizeDeviceName(await storageHelpers.getString(HOUSE_DEVICE_NAME_KEY));
...
export async function setLocalDeviceName(raw: string): Promise<string> {
  ...
  await storageHelpers.setString(HOUSE_DEVICE_NAME_KEY, name);
...
/**
 * How long a device may go without reaching the home before the list stops
 * calling it "Active".
 *
...
/**
 * Is this device still reaching the home?
 *
...
export function describeDevice(input: {
  ...
  // This phone is self-evidently reaching the home — it is the one asking. Its
  // own stamp lands a poll later than the render, so trusting the field here
  // would flash "Not syncing" on the row the member is holding.
