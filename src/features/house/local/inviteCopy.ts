 ...
 *
 * Pure, and in their own module because the two screens that need them are
 ...
 */
export function buildHouseInviteShareText(invite: {
  shortCode: string;
  secret: string;
  link: string;
}): string {
  return [
    'Join my home in Symply House.',
    '',
    ...
    invite.link,
    '',
    'If the link does nothing, open Symply House → Settings → Invite & home and enter the code and secret above.',
  ].join('\n');
}
