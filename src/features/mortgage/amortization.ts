/**
 * Mortgage amortization engine — CLIENT MIRROR of
 * backend/src/services/mortgage/amortization.ts. Used ONLY for the setup-wizard
 * live preview before the mortgage exists server-side; once saved, the FE always
 * trusts the server. Kept byte-for-byte identical to the backend engine (same
 * golden vectors assert both) so preview and server never disagree.
 *
 * DO NOT add imports or diverge the formulas — if you change one engine, change
 * both and re-run GV-1..GV-10 on each side.
 */
